...
const showNews = allNews => {
    ...
    allNews.forEach(news => {
        ...
        <div class="card lg:card-side bg-white shadow-xl">
            <figure><img class="" src="${thumbnail_url}" alt="Album"></figure>
            <div class="card-body">
                    <h2 class="card-title">${title}</h2>
                    <p>${details.length > 350 ? details.slice(0, 350) + '...' : details}</p>
                    <div class="flex flex-col mt-5 lg:flex-row justify-between">
                        
                        <div class="flex gap-1">
                            <img src="${author.img}" alt="" style="border-radius: 50%;"
                            class="h-12 w-12 rounded-full p-0">
                            <p>${author.name ? author.name : 'No data found'} <br>
                            ${author.published_date ? author.published_date : 'No data found'}</p>
